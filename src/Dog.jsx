import {useRef, useState, useEffect, Suspense} from "react";
import {Canvas, useFrame, useLoader} from '@react-three/fiber';
import { Stats, OrbitControls, Environment, useGLTF, Clone } from '@react-three/drei'
import {GLTFLoader} from 'three/addons/loaders/GLTFLoader';
import Barrier from './Barrier';
import Barrier_mid from './Barrier_mid';
import Barrier1 from "./Barrier1";
import Road from './Road';
import Road1 from './Road1';
import Road2 from './Road2';
import Player_2 from "./Player_2";
import {useNavigate} from "react-router-dom";

export default function Dog() {
  const [x_val, upd] = useState(0);
  const [y_val, updy] = useState(0);
  const [pos, set] = useState(2);
  const gltf = useLoader(GLTFLoader, './models/cute_bear.glb');

  const [visible, setVisible] = useState(false);
  let testing = {bar1:0,bar2:0,bar:0}
  const Doggo = () => {
    const ref = useRef();
    useFrame((state, delta) => { 
      if (pos === 1 && testing.bar1 === 1 && y_val <0.15){
        setVisible(false);
      }
      if (pos === 2 && testing.bar === 1 && y_val <0.15){
        setVisible(false);
      }
      if (pos === 3 && testing.bar2 === 1 && y_val <0.15){
        setVisible(false);
      }
      ref.current.position.x = x_val;
      ref.current.position.y = y_val;
      if (y_val > 0){
        ref.current.rotation.x += y_val*1;
        updy(y_val-0.05);
      }
    })
    const detectKeys = (e) => {
      if (e.key === "a"){
        if (pos === 2){
          upd(0.5);
          set(3);
        }
        if (pos === 1){
          upd(0);
          set(2);
        }
      }
      if (e.key === "w"){
        updy(1.3);
      }
      if (e.key === "d") {
        if (pos === 3){
          upd(0);
          set(2);
        }
        if (pos === 2){
          upd(-0.5);
          set(1);
        }
      }
    }
    useEffect(() => {
      document.addEventListener('keydown', detectKeys, true);
    })
    return (
      <mesh ref={ref}
      scale={0.055}>
          <primitive
          object={gltf.scene}
          position={[0,-0.32,-2]}
          children-0-castShadow
        />
      </mesh>
    )
  }


  return (
    <div className="container">

      <div className="dog_holder">
        {visible ? (
        <Canvas camera={{ position: [0, 0.5, -3], near: 0.025 }}>

        <directionalLight
          position={[0,1,-2]}
          castShadow
          intensity={Math.PI * 2}
        />
          <Barrier b_pos={0}
          get_em={testing}/>
          <Barrier_mid
          b_pos={-0.5}
          get_em={testing}/>
          <Barrier1
          b_pos={0.5}
          get_em={testing}/>
          <Doggo/>
          <Player_2/>
          <Road2/>
          <Road/>
          <Road1/>

        </Canvas>): (
        <div className="death_screen">
          <h1> New High Score! </h1>
          <h3> 9121522521 </h3>
          <Canvas camera={{ position: [0, 0.5, 3], near: 0.025 }}>
            <directionalLight
            position={[0,1,2]}
            castShadow
            intensity={Math.PI * 2}
          />
            <mesh
              scale={0.1}>
              <primitive
              object={gltf.scene}
              position={[0,15,-2]}
              children-0-castShadow
            />
            </mesh>
            <OrbitControls/>
          </Canvas>
        </div>
      )}
    </div>
    </div>
)
}

